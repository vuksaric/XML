...
@Component({
  selector: 'app-search-unreg',
  templateUrl: './search-unreg.component.html',
  styleUrls: ['./search-unreg.component.css']
})
export class SearchUnregComponent implements OnInit {
  searchValue: string = "";
  ...
  ngOnInit(): void {
    this.decoded_token = 0;
    this.profileService.getPublicProfiles().subscribe(data=>{console.log(data); this.listOfDataProfiles=data;})
    this.profileService.getProfilesForTagging(this.decoded_token).subscribe(data=>{console.log(data); this.listOfDataTags=data;})
    this.postStory.getLocations(this.decoded_token).subscribe(data=>{console.log(data); this.listOfDataLocations=data;})

  }

  view(item : string){
    this.router.navigate(['view-profile-unregistered/'+item]);
  }
  ...
  viewLocations(item : string){
    ...
    this.postStory.getPostsByLocation(this.decoded_token,item).subscribe(data=>{console.log(data); this.listOfDataLocation=data});
  }
